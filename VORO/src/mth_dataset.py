...
class MTHv2PairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = self.data[idx]
        normalized_feature = [
            i / data["width"] if idx % 2 == 0 else i / data["height"]
            for idx, i in enumerate(data["feature"])
        ]
        sample = {
            "feature": torch.tensor(normalized_feature, dtype=torch.float64),
...
class MTHv2PairImageDataset(Dataset):
    ...
    def __init__(
        ...
        for data in self.data:
            data["feature"] = self.resize_coor(
                data["feature"], (data["width"], data["height"])
            )
        if augmentation:
            # self.augmentation = T.Compose([T.RandomAffine(degrees= 20 ,scale=(0.7,1))])
            self.augmentation = T.Compose(
                [T.RandomAffine(degrees=0, scale=(1, 1), translate=(0, 0.1))]
            )
        else:
        ...
        if self.image_flag:
            ...
            for image in tqdm(set([d["image"] for d in self.data])):
                self.images[image] = np.array(
                    transform(Image.open(image_path + image + ".jpg").convert("L"))
                )  # read all images
    ...
    def __getitem__(self, idx):
        ...
        mask1, mask2 = self.__get_polygon_mask__(poly1), self.__get_polygon_mask__(
            poly2
        )
        ...
        if self.image_flag:
            ...
            feature = torch.tensor(
                np.array([image / 255, mask1 / 255, mask2 / 255]), dtype=torch.float64
            )
        else:
            mask3 = np.zeros((self.image_size, self.image_size))
            feature = torch.tensor(
                np.array([mask3, mask1 / 255, mask2 / 255]), dtype=torch.float64
            )
    ...
    def resize_coor(self, coors, ori_size):
        ...
        """
        x_ratio, y_ratio = float(self.image_size) / float(ori_size[0]), float(
            self.image_size
        ) / float(ori_size[1])
        return [
            round(i * x_ratio) if idx % 2 == 0 else round(i * y_ratio)
            for idx, i in enumerate(coors)
        ]
