...
from mth_dataset import MTHv2PairImageDataset
from torch.utils.data import DataLoader
from tqdm import tqdm
from utils import mkdir, save_log
...
def evaluate(img_seqs):
    ...
    for img in img_seqs:
        ...
        scores["K"].append(
            float(
                metrics.kendall_tau_distance(
                    seq, list(range(len(seq))), normalized=False
                )
            )
        )
        scores["F"].append(
            float(
                metrics.spearman_footrule_distance(
                    seq, list(range(len(seq))), normalized=False
                )
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="CNN classifier for Pair-wise Reading order"
    )
    # model variables
    parser.add_argument(
        "--seed",
    ...
    if not args.eval_only:
        for epoch in range(args.epochs):
            t_loss = train(
                model, tr_dataloader, criterion, optimizer, scheduler, device
            )
            v_loss, preds, idxs, images = validate(
            ...
            print(info_str.format(epoch, f_score, k_score))
            save_log(
                args.out_folder + "log.txt", info_str.format(epoch, f_score, k_score)
            )

            if not epoch % args.echo_rate:
                info_str = "Epoch {} : train-loss: {} val-loss: {}"
                print(info_str.format(epoch, t_loss, v_loss))
                save_log(
                    args.out_folder + "log.txt", info_str.format(epoch, t_loss, v_loss)
                )
            if best_k_score > k_score:
                ...
                torch.save(
                    model.state_dict(), os.path.join(args.out_folder, "model.pth")
                )
    ...
    _, preds, idxs, images = validate(
        model, te_dataloader, None, device, mode="prediction"
    )
    img_seqs = decode_pairs(preds, idxs, images)
    scores = evaluate(img_seqs)
    k_score = np.mean(scores["K"])
